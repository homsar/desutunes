...
# Using nkd.su's artist parser from
# https://github.com/very-scary-scenario/nkd.su/blob/main/nkdsu/apps/vote/parsers.py
...
def parse_artist(string: str, fail_silently: bool = True) -> Iterable[Tuple[bool, str]]:
    ...
    try:
        ...
        if fail_silently:
            logging.debug('problem parsing artist name %r:\n  %s', string, e)
            yield (True, string)
            return
        else:
            raise e
